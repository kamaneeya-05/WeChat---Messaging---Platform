...
const ChatDashboard = () => {
    ...
    if (!realCurrentUser)
        return null;
    return (<div className="flex h-screen w-screen overflow-hidden bg-gray-100">
      <Sidebar conversations={conversations} activeId={activeConversation?._id ?? null} onSelect={selectConversation} currentUser={realCurrentUser}/>

      <main className="flex-1 flex overflow-hidden">
        {activeConversation ? (<ChatWindow conversation={activeConversation} messages={activeMessages} members={activeMembers} onSendMessage={sendMessage}/>) : (<EmptyState />)}
      </main>
    </div>);
};
// --- ROUTE PROTECTION ---
