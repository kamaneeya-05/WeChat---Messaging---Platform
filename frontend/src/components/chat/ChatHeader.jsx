...
export default function ChatHeader({ conversation, onInfoToggle, showInfo, onVoiceCall, onVideoCall }) {
    ...
    return (<div className="h-16 px-5 flex items-center justify-between border-b border-slate-100 bg-white flex-shrink-0">
      <div className="flex items-center gap-3">
        {type === 'group' ? (<div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
        ...
        <div>
          ...
          {isTyping ? (<div className="flex items-center gap-1 text-xs text-green-600">
              <span>typing</span>
      ...
      <div className="flex items-center gap-1 relative" ref={moreMenuRef}>
        ...
        <button onClick={onInfoToggle} className={`p-2 rounded-lg transition ${showInfo ? 'bg-green-50 text-green-600' : 'hover:bg-gray-100 text-gray-500 hover:text-gray-700'}`} title="Conversation info">
          <Info size={17}/>
        </button>
        <button onClick={() => setShowMoreMenu((prev) => !prev)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-gray-700 transition" title="More options">
          <MoreHorizontal size={17}/>
        </button>
        {showMoreMenu && (<div className="absolute right-5 top-14 z-20 min-w-[190px] rounded-lg border border-slate-200 bg-white p-1 shadow-lg">
            <button onClick={toggleMute} className="w-full rounded-md px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-100">
              {isMuted ? 'Unmute chat' : 'Mute chat'}
            </button>
            <button onClick={copyChatId} className="w-full rounded-md px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-100">
              Copy chat ID
            </button>
            <button onClick={() => {
                setShowMoreMenu(false);
        ...
        {copied && (<div className="absolute right-5 top-2 rounded bg-slate-800 px-2 py-1 text-xs text-white">
            Chat ID copied
          </div>)}
      </div>
    </div>);
