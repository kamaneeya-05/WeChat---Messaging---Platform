  ...
  export function useChat() {
    ...
    useEffect(() => {
      ...
      newSocket.emit('setup', (user as any)._id || (user as any).id); 
    });
    ...
    useEffect(() => {
      ...
      const fetchMessages = async () => {
        try {
          ...
          socket.emit('messages read', { chatId: activeConversation._id, readerId: user?._id || (user as any)?.id });
        } catch (error) {
     ...
     const handleNewMessage = (newMessage: any) => {
      ...
      const handleMessagesRead = (payload: { chatId: string }) => {
